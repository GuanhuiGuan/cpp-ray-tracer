...
void sceneTexBalls() {
    ...
    world.add(std::make_shared<Sphere>(Point{1.5, 0.2, 0}, 1.2, 
        std::make_shared<Lambertian>(
            std::make_shared<TexImg>("../resource/image/earthmap.jpg")
        )
    ));
    world.add(std::make_shared<Sphere>(Point{-3, -0.2, -2}, 0.8, 
        std::make_shared<Lambertian>(
            std::make_shared<TexImg>("../resource/image/moon.jpg")
        )
    ));
    world.add(std::make_shared<Sphere>(Point{0, -0.1, 1}, 0.9, 
        std::make_shared<Lambertian>(
            std::make_shared<Noise>(10)
        )
    ));
